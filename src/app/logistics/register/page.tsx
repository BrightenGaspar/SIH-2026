...
import { ArrowLeft, CheckCircle2, Languages, Truck, AlertCircle } from 'lucide-react';
...
export default function LogisticsRegisterPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col justify-between py-8 px-4 sm:px-6 lg:px-8 selection:bg-amber-500 selection:text-slate-950 font-sans">
      ...
      <div className="max-w-xl w-full mx-auto my-6">
        <Card className="bg-white border border-slate-200 p-8 shadow-xs rounded-2xl">
          
          <div className="text-center mb-6">
            <div className="w-14 h-14 rounded-2xl bg-amber-50 text-amber-600 border border-amber-200 flex items-center justify-center mx-auto mb-3 font-black text-2xl">
              <Truck className="w-7 h-7 text-amber-600" />
            </div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">{t('createAccount')}</h1>
            <p className="text-xs text-slate-500 mt-1">Join the AgriFlow cold-chain freight & return haul network.</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {step === 1 && (
              <div className="space-y-4 animate-in fade-in">
                <div>
                  <label className="text-xs font-bold text-slate-700 block mb-1">Carrier / Operator Name *</label>
                  <input
                    type="text"
                    {...register('fullName')}
                    className="w-full bg-slate-50 border border-slate-200 focus:border-amber-500 focus:bg-white rounded-xl px-4 py-2.5 text-sm text-slate-900 outline-none transition"
                  />
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 border border-slate-200 focus:border-amber-500 focus:bg-white rounded-xl px-4 py-2.5 text-sm text-slate-900 outline-none font-bold transition"
                  >
                ...
                <Button type="button" onClick={nextStep} className="w-full py-3 mt-4 bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold">
            ...
            {step === 2 && (
              <div className="space-y-4 animate-in fade-in">
                ...
                <div>
                  <label className="text-xs font-bold text-slate-700 block mb-1">{t('place')} (Fleet Base Hub) *</label>
                  <input
                    type="text"
                    {...register('place')}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="text-xs font-bold text-slate-700 block mb-1">{t('vehicleNumber')} *</label>
                    <input
                      type="text"
                      ...
                      className="w-full bg-slate-50 border border-slate-200 focus:border-amber-500 focus:bg-white rounded-xl px-4 py-2.5 text-sm text-slate-900 outline-none transition"
                    />
                    {errors.vehicleNumber && <p className="text-[11px] text-rose-500 mt-1">{errors.vehicleNumber.message}</p>}
                  </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-bold text-slate-700 block mb-1">{t('carrierCapacity')} (kg) *</label>
                    <input
                      type="number"
                ...
                <div className="flex gap-3 mt-4">
                  ...
                  <Button type="button" onClick={nextStep} className="flex-1 bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold">
                    {t('next')} &rarr;
                  </Button>
                </div>
              </div>
            ...
            {step === 3 && (
              <div className="space-y-4 animate-in fade-in">
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-slate-800 space-y-1">
                  <span className="font-bold text-amber-700 block mb-1">✓ Fleet Setup Ready</span>
                ...
                <div className="flex gap-3 mt-4">
                  ...
                  <Button type="submit" isLoading={isSubmitting} className="flex-1 bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold">
          ...
          <div className="mt-6 pt-4 border-t border-slate-100 text-center text-xs text-slate-500">
            ...
            <Link href="/logistics/login" className="text-amber-600 font-bold hover:underline">
              {t('login')} Here
            </Link>
          </div>
