...
export const farmerService = {
   ...
   * based on the logged-in farmer profile context.
   */
  async getProduceList(farmerId?: string): Promise<Produce[]> {
    try {
      ...
      if (error) {
        console.error('Supabase query error in getProduceList:', error.message);
        return [];
      }
      ...
      return data.map((row: any): Produce => {
      ...
      console.error('Error fetching produce from Supabase:', err?.message);
      return [];
    }
  },
   ...
   * linked to the logged-in farmer profile context.
   */
  async addProduce(
    ...
    if (
      ...
      // Check if profile exists in public.profiles to satisfy FK
    ...
    const locationToSave = item.location || ctx?.location || 'Regional Farm Hub';
    ...
    if (error || !data) {
      console.error('Supabase insert error in addProduce:', error?.message);
      throw new Error(error?.message || 'Failed to add produce listing to database');
    }
